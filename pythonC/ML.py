...
pallete_path = "C:/Users/USER/Desktop/Shoes-variation-server/p_test/"
...
drive_path = "C:/Users/USER/Desktop/Shoes-variation-server/DesignData/"
...
#0은 확인을 위한 데이터 0빈은 빈 데이터
'''for i in range(1,dataNum+1):
    print(i,"\n")
    print(temp_data_RGB[i])
    print("")'''
...
#print(data_LAB_all_cluster)

pallete_LAB_cluster = pallete_LAB.copy() #영역 없이 Clustering

#k=47
#km = KMeans(n_clusters=12, init='k-means++', n_init=10, max_iter=300, tol=1e-04, random_state=1)
#y_km = km.fit_predict(data_LAB_all)

#data_LAB_all_cluster['cluster']=y_km #무슨 클러스터인지 열 추가

all_LAB_SVM = SVC(kernel='linear', C=0.42, gamma = 0.1) #SVM 생성
all_LAB_SVM = all_LAB_SVM.fit(data_LAB_all_cluster.drop(columns=['cluster']),data_LAB_all_cluster['cluster']) #이걸로 SVM 생성
